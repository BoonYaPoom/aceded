...
class BookController extends Controller
{
    ...
    public function store(Request $request, $department_id, $category_id)
    {
        try {
            ...
            $books->detail = '';
            $lastSort = Book::where('category_id', $category_id)->max('book_sort');
            $newSort =  $lastSort + 1;
            $books->book_date = now();
            ...
            if ($request->hasFile('bookfile')) {
                ...
                if ($request->book_type == 0) {
                    ...
                    if ($request->hasFile('bookfile')) {
                        ...
                        Storage::disk('sftp')->put($uploadDirectory . '/' . $image_bookfile, file_get_contents($request->cover->getRealPath()));
            ...
            if ($request->hasFile('bookfile')) {
                if ($request->book_type == 0) {
                    ...
                    if ($request->hasFile('bookfile')) {
                        ...
                        Storage::disk('sftp')->put($uploadDirectory . '/' . $image_bookfile, file_get_contents($request->bookfile->getRealPath()));
                        $books->bookfile = 'upload/Book/' . $books->book_id . '/' . $image_bookfile;
                    }
                }
            }
    ...
    public function changeSortbooks(Request $request)
    {
        $books = Book::find($request->book_id);

        if ($books) {
            $books->book_sort = $request->book_sort;
            $books->save();

            return response()->json(['message' => 'สถานะถูกเปลี่ยนแปลงเรียบร้อยแล้ว']);
        } else {
            return response()->json(['message' => 'ไม่พบข้อมูล books']);
        }
    }
}
