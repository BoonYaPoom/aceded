...
session_start();	
	...
	if($_SESSION['BACKEND']==1){
		if(!empty($_SESSION['subject_id'])){
			$editor_upload_path="../../../../upload-subject/".substr(md5($_SESSION["subject_id"],0,15));
			if(!is_dir($editor_upload_path)&&!empty($_SESSION["subject_id"]))  {
				mkdir($editor_upload_path);
				file_put_contents($editor_upload_path.'/index.html','Directory access is forbidden.');
			}
		}else if(!empty($_SESSION['course_id'])){
			$editor_upload_path="../../../../upload-course/".substr(md5($_SESSION["course_id"],0,15));
			if(!is_dir($editor_upload_path)&&!empty($_SESSION["course_id"]))  {
				mkdir($editor_upload_path);
				file_put_contents($editor_upload_path.'/index.html','Directory access is forbidden.');
			}
		}else{
			$editor_upload_path="../../../../upload-backend/".substr(md5($_SESSION["logged_in"]["uid"],0,15));
			if(!is_dir($editor_upload_path)&&!empty($_SESSION["logged_in"]["uid"]))  {
				mkdir($editor_upload_path);
				file_put_contents($editor_upload_path.'/index.html','Directory access is forbidden.');
			}
		}
		...
		$editor_upload_path="../../../../upload-frontend/".substr(md5($_SESSION["logged_in"]["uid"],0,15));
		if(!is_dir($editor_upload_path)&&!empty($_SESSION["logged_in"]["uid"]))  {
			mkdir($editor_upload_path);
			file_put_contents($editor_upload_path.'/index.html','Directory access is forbidden.');
		}
	}
	define('CONFIG_SYS_DEFAULT_PATH', $editor_upload_path); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', $editor_upload_path);	//accept relative path only
	...
	define('CONFIG_DEFAULT_VIEW', (CONFIG_SYS_THUMBNAIL_VIEW_ENABLE?'thumbnail':'thumbnail')); //thumbnail or detail
