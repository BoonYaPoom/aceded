...
function showTree($arrays,$array,$data,$parent=0,$level=0,$lesson=''){
					$n=0;
					if(empty($level)) $sublesson=""; else $sublesson="small";
					$level++;
					if(is_array($arrays[$parent])) {
						foreach($arrays[$parent] as $lesson_id => $lesson_th){
							...
							if(!empty($parent)&&!empty($lesson)) $showlesson=$lesson."."; else $showlesson="";
							$lessonnum=$showlesson.$n;
							if(count($arrays[$lesson_id])>0){ $lesson=$lessonnum; }
							$prefix=md5('moc'.date('Ymd'));
							$left=($level-1)*25;
							$checked=(!empty($row['lesson_status']))?"checked":"";
							$iconcolor=(!empty($row['content_path']))?'text-success':'text-muted';
							if(count($arrays[$lesson_id])==0){  $icon1=$icon2='<i class="fas fa-minus fa-xs text-success " ></i>'; }else{ $icon1='<i class="fas fa-minus-circle text-success " style="cursor:pointer" id="icon1_'.$lesson_id.'" onclick="togglerows('.$lesson_id.');"></i>';$icon2='<i class="fa fas fa-minus-circle text-success pointer" style="cursor:pointer" id="icon2_'.$lesson_id.'" onclick="togglerows('.$lesson_id.');"></i>';}
							  echo'<!-- tr -->
							  <tr id="'.$prefix.'__course_lesson__lesson_status__lesson_id__'.$lesson_id.'__'.time().'__row"  class="rows_'.$parent.' '.$display.' '.$sublesson.'" >
								<!--<td class="">'.$data['lesson_type'][$row['lesson_type']]['type_th'].'</td> -->
								...
								<td style="padding-left:'.$left.'px">'.$icon1.' '.$lesson_th.'</td>
								 ...
								 <td><i style="cursor:pointer;" class="'.$data['content_type'][$row['content_type']]['icon'].' fa-lg '.$iconcolor.' switcher-upload" data-toggle="tooltip" title="'.$data['content_type'][$row['content_type']]['content_th'].'" id="'.$lesson_id.'"></i></td>
								<td class="align-middle" s> <label class="switcher-control switcher-control-success switcher-control-lg"><input type="checkbox" class="switcher-input switcher-edit" '.$checked.' value="1" id="'.$prefix.'__course_lesson__lesson_status__lesson_id__'.$lesson_id.'__'.time().'"> <span class="switcher-indicator"></span> <span class="switcher-label-on">ON</span> <span class="switcher-label-off text-red">OFF</span></label> </td>';
								...
								<a href="'.base_url().'lms/addsub_lessonform/'.$row['subject_id'].'/'.$lesson_id.'.html" data-toggle="tooltip" title="เพิ่มย่อย"><i class="fas fa-plus-circle fa-lg text-danger"></i></a>
								 <a href="'.base_url().'lms/lessonform/'.$lesson_id.'.html" data-toggle="tooltip" title="แก้ไข"><i class="far fa-edit fa-lg text-success"></i></a>
								 <a href="#clientDeleteModal"  id="'.$prefix.'__course_lesson__lesson_status__lesson_id__'.$lesson_id.'__'.time().'" rel="'.strip_tags($row['lesson_th']).'" class="switcher-delete" data-toggle="tooltip" title="ลบ"><i class="fas fa-trash-alt fa-lg text-warning "></i></a>
								</td>
							  </tr><!-- /tr -->';
